...
- mapping_launch.py: Provides /odometry/filtered (EKF sensor fusion from IMU + RTAB-Map)

Topics:
- /odometry/filtered: Robot pose (used to detect first movement)
- /cmd_vel_manual: Manual control commands (monitored for compliance)
"""
...
class AutonomousPhaseManager(Node):
    ...
    def __init__(self):
        ...
        self.odom_sub = self.create_subscription(
            Odometry,
            '/odometry/filtered',  # From EKF sensor fusion (mapping_launch.py)
            self.odom_callback,
            reliable_qos
        )
