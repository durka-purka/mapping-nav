...
- mapping_launch.py: Provides /odometry/filtered (EKF sensor fusion)
- navigation_launch.py: Provides Nav2 action servers (navigate_to_pose)

Frames:
- map: Global frame from RTAB-Map SLAM (mapping_launch.py)
- base_link: Robot center (broadcast by mapping_launch.py)
- start_frame: Created by start_frame_reference.py

Topics:
- /odometry/filtered: Robot pose from EKF (mapping_launch.py)
- /navigate_to_pose: Nav2 action server (navigation_launch.py)
"""
...
class WaypointHandler(Node):
    ...
    def __init__(self):
        ...
        self.odom_sub = self.create_subscription(
            Odometry,
            '/odometry/filtered',  # EKF-fused odometry from robot_localization (mapping_launch.py)
            self.odom_callback,
            reliable_qos
        )
    ...
    def navigate_to_next_landmark(self) -> bool:
        ...
        goal.pose.header.frame_id = 'map'  # Global frame from RTAB-Map (mapping_launch.py)
