...
- mapping_launch.py: Provides TF tree (map, base_link, etc.) and EKF
- nav2_complete.launch.py: Start Frame Reference Manager (creates start_frame)

Coordinate Transformation Pipeline:
1. Perception module detects objects in map frame
2. Object Reporter receives detections on /perception/* topics
3. TF Buffer transforms from map → start_frame
4. Reports published with all coordinates in start_frame

Frames:
- map: Global frame from RTAB-Map (mapping_launch.py)
- start_frame: Robot's initial position as origin (start_frame_reference.py)

Topics:
- /perception/cube_detections: Input detection messages (map frame)
- /perception/landmark_detections: Input landmark messages (map frame)  
- /object_reporter/cube_report: Output with start_frame coordinates
- /object_reporter/landmark_report: Output with start_frame coordinates
"""
...
class ObjectReporter(Node):
    ...
    def __init__(self):
        ...
        self.cube_detection_sub = self.create_subscription(
            String,
            '/perception/cube_detections',  # {color, x, y, z, confidence} in map frame
        ...
        self.landmark_detection_sub = self.create_subscription(
            String,
            '/perception/landmark_detections',  # {id, text, x, y} in map frame
